...
def infosource(request):
    return render(request, "encyclopedia/infosource.html", {
        "entries": util.list_entries()
    })


def howto(request):
    return render(request, "encyclopedia/howto.html", {
        "entries": util.list_entries()
    })
...
def search(request):
    if request.method == 'POST':
        ...
        if len(searchlist) == 0:  # searchlist is empty
            return render(request, "encyclopedia/error.html", {
                'error_message': f' Oops! looks like no results for \'{term}\' '
            })
